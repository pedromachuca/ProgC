...
// Remplir le corps de la fonction InvCipher_Mode_2() puis déchiffrer le fichier "chiffre3"
// en utilisant le choix 6 du menu. Quel est le mode opératoire employé dans Cipher_Mode_2() ?
void InvCipher_Mode_2 (FILE * FIC_IN, FILE * FIC_OUT)
{
  ...
  while (continuer==1)
  {
		...
		InvCipher(); //prend in[] sort out[]; in doit être le chiffré decoupe
