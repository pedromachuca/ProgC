...
double indice_coincidence( int * texte, int longueur_texte)
{
	...
	for(int i=0; i<26; i++){
		...
		IC += ((double)nb_lettre*(double)(nb_lettre-1))/((double)longueur_texte*((double)(longueur_texte-1)));
	}	
  /*
   * 
   * 
...
/*On tient compte de la période et du décalage à effectuer à chaque fois pour bien se positionner
 periode désigne une taille potentielle de la clé et indice_dec le décalage interne dans la clé à partir duquel on va extraire un sous texte*/
void extraction_sous_texte( int * texte, int * sous_texte, int longueur_sous_texte, int periode, int indice_dec)
{
	for(int i=0; i<longueur_sous_texte; i++){
...
void analyse_coincidence(int *texte_chiffre, int longueur_texte)
{
  ...
  while(strcmp(commande,"exit") != 0)
  {
	  ...
	  for (periode=1; periode<=Tranche_Periode; periode++)
	  {
		  ...
		  extraction_sous_texte(texte_chiffre, sous_texte, longueur_sous_texte, periode, i);//Pour extraire le sous-texte correspondant
	  ...
	  if( (0 <= j) && (j < periode))
	    {
	      ...
	      extraction_sous_texte(texte_chiffre, sous_texte, longueur_sous_texte, periode, j);
